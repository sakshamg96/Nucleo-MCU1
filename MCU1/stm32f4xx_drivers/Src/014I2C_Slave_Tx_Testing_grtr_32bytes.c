...
void I2C_ApplicationEventCallback(I2C_Handle_t* pI2CHandle, uint8_t AppEvent) {
	...
	char send_msg[] = "STM32 Slave is replying multiple times to exceed 32 byte limit and split read transaction into multiple transactions.";
	uint32_t data_len = strlen(send_msg);
	...
	if(AppEvent == I2C_EV_DATA_REQ) {
		...
		if(command_code == 0x51) {
			...
			//Send contents of send_msg
			I2C_SlaveSendData(pI2CHandle->pI2Cx, send_msg[wr_ptr]);
			wr_ptr++;
		}
		...
		if(command_code != 0x52) {
			...
			//To transmit 33rd byte correctly, send data is being called twice(1-during 32nd data byte 2-after address phase of next read transaction)
			wr_ptr--;
			//Data will be received in chunks of 32 bytes with single read txn containing 32 bytes or less
			if(wr_ptr >= data_len) {
				wr_ptr = 0;
				command_code = 0;
			}
